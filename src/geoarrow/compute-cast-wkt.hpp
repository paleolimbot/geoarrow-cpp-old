    ...
    if (stack_.size() == 0 ||
      ...
      switch (geometry_type) {
        ...
        case util::GeometryType::GEOMETRYCOLLECTION:
          ...
          break;
        default:
          throw util::IOException("Don't know how to name geometry type %d",
                                  geometry_type);
      }
 ...
 private:
  ...
  void write_coord(double value) {
    ...
    int n_needed = geoarrow_d2s_fixed_n(
        value, significant_digits_, reinterpret_cast<char*>(data_at_cursor));
    string_builder_.advance_data(n_needed);
  }

  void write_string(const char* value) {
    string_builder_.write_buffer(reinterpret_cast<const uint8_t*>(value),
                                 strlen(value));
  }
