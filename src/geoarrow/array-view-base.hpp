...
// a sequence of `struct ArrowArray`s, both of which must be valid pointers for
// the lifetime of the `ArrayView`. The `ArrayView` supports handler-style
// iteration using a `Handler` and pull-style iteration using virtual methods.
// The handler-style iteration is particularly useful as a way to write
// general-purpose code without using virtual methods.
class ArrayView {
 public:
...
template <class TArrayView>
Handler::Result read_feature_templ(TArrayView& view, int64_t offset,
                                   Handler* handler) {
  Handler::Result result;
