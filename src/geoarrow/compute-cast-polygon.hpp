...
class PolygonArrayBuilder : public ComputeBuilder {
 public:
  PolygonArrayBuilder(const ComputeOptions& options = ComputeOptions())
  ...
  Result geom_start(util::GeometryType geometry_type, int32_t size) {
    if (size == 0) {
      ...
      throw util::IOException("Can't write non-polygon (%d[%d]) as POLYGON",
                              geometry_type, size);
    }
  }
 ...
 private:
  arrow::hpp::builder::ListArrayBuilder<
      arrow::hpp::builder::ListArrayBuilder<PointArrayBuilder>>
      builder_;
};
