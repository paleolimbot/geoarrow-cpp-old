...
class Meta {
 public:
  ...
  void reset_error() { memset(error_, 0, 1024); }
  ...
  bool schema_valid(const struct ArrowSchema* schema) {
    switch (storage_type_) {
      case util::StorageType::FixedSizeList:
        if (fixed_width_ <= 0) {
          set_error("Expected fixed-size list to have size > 0 but got %lld",
                    fixed_width_);
          return false;
        }
      ...
      case util::StorageType::LargeList:
        if (schema->n_children != 1) {
          set_error(
              "Expected container schema to have one child but found %lld",
              schema->n_children);
          return false;
        }
        break;
      case util::StorageType::FixedWidthBinary:
        if (fixed_width_ <= 0) {
          set_error(
              "Expected fixed-width binary to have width > 0 but got %lld",
              fixed_width_);
          return false;
        }
      default:
    ...
    switch (extension_) {
        ...
        switch (storage_type_) {
          case util::StorageType::FixedSizeList:
            ...
            if (fixed_width_ != static_cast<int>(strlen(dim_))) {
              set_error(
                  "Expected geoarrow.point with dimensions '%s' to have width "
                  "%d but found width %lld",
                  dim_, strlen(dim_), fixed_width_);
              return false;
            }
            if (!child.set_schema(schema->children[0])) {
              set_child_error(child.error_,
                              "geoarrow.point has an invalid child schema");
              return false;
            }
            if (child.storage_type_ != util::StorageType::Float64) {
              set_error(
                  "Expected child of fixed-width list geoarrow.point to have "
                  "type Float64");
              return false;
            }
            break;
          default:
            set_error(
                "Expected geoarrow.point to be a fixed-width list but found "
                "'%s'",
                schema->format);
            return false;
        }
        ...
        switch (storage_type_) {
          case util::StorageType::List:
            ...
            if (child.storage_type_ != util::StorageType::List) {
              set_error(
                  "Expected child of a geoarrow.polygon to be a list but found "
                  "'%s'",
          ...
          default:
            set_error("Expected geoarrow.polygon to be a list but found '%s'",
                      schema->format);
            return false;
        }
        ...
        switch (storage_type_) {
          case util::StorageType::List:
            ...
            switch (child.extension_) {
              ...
              default:
                set_error(
                    "Child of geoarrow.geometrycollection must be a "
                    "geoarrow.point, geoarrow.linestring, or geoarrow.polygon");
                return false;
            }
          ...
          default:
            set_error(
                "Expected geoarrow.geometrycollection to be a list but found "
                "'%s'",
                schema->format);
            return false;
        }
      ...
      case util::Extension::WKB:
        ...
        switch (storage_type_) {
          ...
          default:
            set_error(
                "Expected geoarrow.wkb to be a binary, large binary, or "
                "fixed-width binary but found '%s'",
                schema->format);
        }
        break;
      ...
      case util::Extension::WKT:
        ...
        switch (storage_type_) {
          ...
          default:
            set_error(
                "Expected geoarrow.wkt to be a string or large string but "
                "found '%s'",
                schema->format);
        }
        break;
  ...
  bool array_valid(const struct ArrowArray* array) {
    if (array->n_buffers != expected_buffers_) {
      set_error("Expected array with %d buffers but found %lld",
    ...
    switch (storage_type_) {
      ...
      case util::StorageType::LargeList:
        if (array->n_children != 1) {
          set_error("Expected container array to have one child but found %lld",
                    array->n_children);
          return false;
        }
        break;
  ...
  void walk_metadata(const char* metadata) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (name_len >= 20 && strncmp(name, "ARROW:extension:name", 20) == 0) {
        ...
        if (value_len >= 14 && strncmp(value, "geoarrow.point", 14) == 0) {
        ...
        } else if (value_len >= 19 &&
                   strncmp(value, "geoarrow.linestring", 19) == 0) {
          extension_ = util::Extension::Linestring;
        } else if (value_len >= 16 &&
                   strncmp(value, "geoarrow.polygon", 16) == 0) {
          extension_ = util::Extension::Polygon;
        } else if (value_len >= 19 &&
                   strncmp(value, "geoarrow.multipoint", 19) == 0) {
          extension_ = util::Extension::MultiPoint;
        } else if (value_len >= 24 &&
                   strncmp(value, "geoarrow.multilinestring", 24) == 0) {
          extension_ = util::Extension::MultiLinestring;
        } else if (value_len >= 21 &&
                   strncmp(value, "geoarrow.multipolygon", 21) == 0) {
          extension_ = util::Extension::MultiPolygon;
        } else if (value_len >= 14 &&
                   strncmp(value, "geoarrow.geometrycollection", 14) == 0) {
          ...
          extension_ = util::Extension::ExtensionOther;
                 ...
                 strncmp(name, "ARROW:extension:metadata", 24) == 0) {
          ...
          if (name_len >= 3 && strncmp(name, "crs", 3) == 0) {
            ...
            if (value_len >= 9 && strncmp(value, "spherical", 9) == 0) {
              edges_ = util::Edges::Spherical;
                       ...
                       strncmp(value, "ellipsoidal", 11) == 0) {
