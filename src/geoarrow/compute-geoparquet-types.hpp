...
struct type_dim_pair_hash {
  std::size_t operator()(
      const std::pair<util::GeometryType, util::Dimensions>& pair) const {
...
class GeoParquetTypeCollector : public ComputeBuilder {
 public:
  ...
  void new_dimensions(util::Dimensions dim) { dim_ = dim; }
 ...
 private:
  ...
  std::unordered_set<std::pair<util::GeometryType, util::Dimensions>,
                     internal::type_dim_pair_hash>
      all_types_;
  std::unordered_set<std::pair<util::GeometryType, util::Dimensions>,
                     internal::type_dim_pair_hash>
      empty_types_;
