...
class FixedSizeListArrayBuilder : public ArrayBuilder {
 public:
  ...
  void set_item_size(int64_t item_size) {
    if (item_size_ == -1) {
      ...
      throw util::Exception(
          "Attempt to resize a fixed-size list from %lld to %lld", item_size_,
          item_size);
    }
  }
  ...
  void release(struct ArrowArray* array_data, struct ArrowSchema* schema) {
    ...
    child_builder_.release(finalizer.array_data.children[0],
  ...
  const char* get_format() {
    ...
    snprintf(format_, sizeof(format_), "+w:%lld",
             static_cast<long long int>(item_size_));
    return format_;
  }
...
class ListArrayBuilder : public ArrayBuilder {
 public:
  ListArrayBuilder() { offset_buffer_builder_.write_element(0); }
  ...
  const char* get_format() { return "+l"; }
