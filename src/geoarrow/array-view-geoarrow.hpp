...
class PointArrayView : public ArrayView {
 public:
  ...
  void set_array(const struct ArrowArray* array) {
    ...
    data_buffer_ =
        reinterpret_cast<const double*>(array->children[0]->buffers[1]);
  }
  ...
  Handler::Result read_coords(Handler* handler, int64_t offset, int64_t n) {
    Handler::Result result;
    HANDLE_OR_RETURN(
        handler->coords(data_buffer_ + (offset + array_->offset) * coord_size_,
                        n, coord_size_));
    return Handler::Result::CONTINUE;
  }
...
class LinestringArrayView : public ListArrayView<PointArrayView> {
 public:
  LinestringArrayView(struct ArrowSchema* schema)
      : ListArrayView<PointArrayView>(schema) {}
  ...
  Handler::Result read_feature(Handler* handler, int64_t offset) {
    return internal::read_feature_templ<LinestringArrayView>(*this, offset,
                                                             handler);
  }
  ...
  Handler::Result read_geometry(Handler* handler, int64_t offset) {
    ...
    HANDLE_OR_RETURN(
        this->child_.read_coords(handler, initial_child_offset, size));
    HANDLE_OR_RETURN(handler->geom_end());
    return Handler::Result::CONTINUE;
  }
};
...
class PolygonArrayView : public ListArrayView<ListArrayView<PointArrayView>> {
 public:
  ...
  Handler::Result read_feature(Handler* handler, int64_t offset) {
    return internal::read_feature_templ<PolygonArrayView>(*this, offset,
                                                          handler);
  }
  ...
  Handler::Result read_geometry(Handler* handler, int64_t offset) {
    ...
    for (int64_t i = 0; i < size; i++) {
      int64_t initial_coord_offset =
          this->child_.child_offset(initial_child_offset + i);
      int64_t ring_size = this->child_.child_size(initial_child_offset + i);

      HANDLE_OR_RETURN(handler->ring_start(ring_size));
      HANDLE_OR_RETURN(this->child_.child_.read_coords(
...
class CollectionArrayView : public ListArrayView<ChildView> {
 public:
  CollectionArrayView(struct ArrowSchema* schema)
      : ListArrayView<ChildView>(schema) {}
  ...
  Handler::Result read_feature(Handler* handler, int64_t offset) {
    return internal::read_feature_templ<CollectionArrayView>(*this, offset,
                                                             handler);
  }
