...
class WKBArrayBuilder : public ComputeBuilder {
 public:
  ...
  void release(struct ArrowArray* array_data, struct ArrowSchema* schema) {
    ...
    string_builder_.set_metadata("ARROW:extension:metadata",
                                 Metadata().build());
  ...
  const char* get_format() { return string_builder_.get_format(); }
  ...
  Result geom_start(util::GeometryType geometry_type, int32_t size) {
    ...
    memcpy(string_builder_.data_at_cursor(), &geometry_type_to_write,
           sizeof(uint32_t));
  ...
  Result ring_start(int32_t size) {
    ...
    stack_.push_back(
        State{util::GeometryType::LINESTRING, string_builder_.data_size(), 0});
    string_builder_.advance_data(sizeof(uint32_t));
  ...
  Result ring_end() {
    if (stack_.size() > 0) {
      memcpy(
          ...
          &stack_.back().size, sizeof(uint32_t));
      stack_.pop_back();
    }
    return Result::CONTINUE;
  }
  ...
  Result geom_end() {
    if (stack_.size() > 0 &&
        stack_.back().geometry_type != util::GeometryType::POINT) {
      memcpy(
          string_builder_.mutable_data() + stack_.back().data_buffer_position,
          &stack_.back().size, sizeof(uint32_t));
    }
