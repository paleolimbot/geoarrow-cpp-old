...
template <typename Child, util::GeometryType ParentType,
          util::GeometryType ChildType>
class MultiArrayBuilder : public ComputeBuilder {
 public:
  ...
  Result ring_start(int32_t size) { return builder_.child().ring_start(size); }
  ...
  Result geom_start(util::GeometryType geometry_type, int32_t size) {
    ...
    if (geometry_type == ParentType) {
      ...
      throw util::IOException("Can't write %d[%d] as %d", geometry_type, size,
                              ParentType);
    }
  }
  ...
  Result ring_end() { return builder_.child().ring_end(); }
  ...
  void release(struct ArrowArray* array_data, struct ArrowSchema* schema) {
    ...
    switch (ParentType) {
      case util::GeometryType::MULTIPOINT:
        ...
        break;
      case util::GeometryType::MULTILINESTRING:
        builder_.set_metadata("ARROW:extension:name",
...
using MultiPointArrayBuilder =
    MultiArrayBuilder<PointArrayBuilder, util::GeometryType::MULTIPOINT,
                      util::GeometryType::POINT>;

using MultiLinestringArrayBuilder =
    MultiArrayBuilder<LinestringArrayBuilder,
                      util::GeometryType::MULTILINESTRING,
                      util::GeometryType::LINESTRING>;

using MultiPolygonArrayBuilder =
    MultiArrayBuilder<PolygonArrayBuilder, util::GeometryType::MULTIPOLYGON,
                      util::GeometryType::POLYGON>;
