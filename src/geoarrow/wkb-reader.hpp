...
class WKBReader {
 ...
 private:
  ...
  Handler::Result read_geometry(Handler* handler) {
    ...
    switch (geometry_type_enum) {
      ...
      case util::GeometryType::GEOMETRYCOLLECTION:
        ...
        break;
      default:
        throw util::IOException("Unrecognized geometry type: %d",
                                geometry_type);
    }
  ...
  Handler::Result read_coords(Handler* handler, uint32_t n,
                              int32_t coord_size) {
  ...
  void read_endian() { swapping_ = read_uint8() != _GEOARROW_ENDIAN; }
  ...
  void check_buffer(int64_t n) {
    if ((offset_ + n) > size_) {
      throw util::IOException("Unexpected end of buffer at %lld + %lld / %lld",
                              offset_, n, size_);
    }
  }
};
